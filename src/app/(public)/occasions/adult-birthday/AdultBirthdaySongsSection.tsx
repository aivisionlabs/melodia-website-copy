...
export default function AdultBirthdaySongsSection({
  songs,
}: {
  songs: Song[];
}) {
  ...
  return (
    <HorizontalScrollSection
      title="Birthday Anthems"
      subtitle="Personalized songs to make their day unforgettable"
      ...
      {songs.length === 0 ? (
        ...
        >
          <p className="text-sm text-text-teal/70 font-body">
            Songs will appear here once they are tagged with the adult-birthday
            category.
          </p>
        </div>
      ) : (
        songs.map((song, index) => (
          <SongLibraryCard
            ...
            song={song}
            index={index}
            pageContext="occasions_adult_birthday"
            ...
            variant="carousel"
            analyticsLocation="occasions_adult_birthday_strip"
            shareMethod="occasions_adult_birthday_strip_share"
          />
        ))
      )}
    </HorizontalScrollSection>
